...
export default function App() {
  const [currentCard, setCurrentCard] = useState<'fold' | 'story'>('fold');
  ...
  const nextCard = () => {
    ...
    setCurrentCard(templates[nextIndex].id as 'fold' | 'story');
  };

  const prevCard = () => {
    const prevIndex = (currentIndex - 1 + templates.length) % templates.length;
    setCurrentCard(templates[prevIndex].id as 'fold' | 'story');
  };
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-full px-6 py-3 shadow-lg">
          <div className="flex items-center gap-4">
            {templates.map((template) => (
              <button
                key={template.id}
                onClick={() => setCurrentCard(template.id as 'fold' | 'story')}
